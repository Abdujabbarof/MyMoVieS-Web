  ...
  useEffect(() => {
    setData(
      likedItems.filter((item) => {
        ...
        const nameOriginal = item?.nameOriginal?.toLocaleLowerCase() || '';
        const nameEn = item?.nameEn?.toLocaleLowerCase() || '';

        return nameRu.includes(searchLower) || nameEn.includes(searchLower) || nameOriginal.includes(searchLower);
      })
    );
  }, [search, likedItems]);
